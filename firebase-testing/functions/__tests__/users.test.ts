...
describe("users", () => {
  describe("get", () => {
    describe("if a user is not authenticated", () => {
      ...
      beforeEach(async () => {
        await testEnv.withSecurityRulesDisabled(async (context) => {
          await userImageRef(context.storage(), userId, "icon.png")
    ...
    describe("if a user is authenticated", () => {
      ...
      beforeEach(async () => {
        await testEnv.withSecurityRulesDisabled(async (context) => {
          await userImageRef(context.storage(), userId, "icon.png").put(
              loadIconImage()
          );
        });
      });
      ...
      describe("another user tries to get user's image", () => {
        test("cannot get image", async () => {
          ...
          await assertFails(
              userImageRef(
                  testEnv.authenticatedContext(anotherUserId).storage(),
                  userId,
                  "icon.png"
              ).getDownloadURL()
          );
    ...
    describe("if a user is not authenticated", () => {
      ...
      beforeEach(async () => {
        await testEnv.withSecurityRulesDisabled(async (context) => {
          ...
          await userImageRef(storage, userId, "icon1.png").put(
              loadIconImage(),
              {contentType}
          );
          await userImageRef(storage, userId, "icon2.png").put(
              loadIconImage(),
              {contentType}
          );
        });
  ...
  describe("create", () => {
    ...
    describe("if a user is authenticated", () => {
      ...
      describe("無効なコンテントタイプの画像をユーザのディレクトリに置こうとする", () => {
        test("cannot put am image", async () => {
          await assertFails(
              userImageRef(
                  testEnv.authenticatedContext(userId).storage(),
      ...
      describe("無効な拡張子を持つ画像をユーザーのディレクトリに置こうとする", () => {
        test("cannot put am image", async () => {
          await assertFails(
              userImageRef(
                  testEnv.authenticatedContext(userId).storage(),
  ...
  describe("update", () => {
    describe("if a user is not authenticated", () => {
      ...
      beforeEach(async () => {
        await testEnv.withSecurityRulesDisabled(async (context) => {
          const storage = context.storage();
          await userImageRef(storage, userId, "icon.png").put(loadIconImage(), {
            contentType,
          });
        });
      });
